...
// Entrada: { phone, text, pushName } | Saída: { phone, reply, mode, userText }
...
const APIKEY = $env.PETFLOW_API_KEY;
...
for (const item of $input.all()) {
  const { phone, text, pushName } = item.json;
  const lower = text.toLowerCase().trim();
  let reply = null, mode = 'polish';

  try {
    // --- Unidade (via api key) ---
    const units = await req('GET', '/rest/v1/units?api_key=eq.' + APIKEY + '&is_deleted=eq.false&select=id,name,timezone');
    const unit = units[0];
    if (!unit) { results.push({ json: { phone, reply: 'Loja não configurada.', mode: 'fixed' } }); continue; }
    ...
    if (['menu', 'inicio', 'início', 'voltar', 'oi', 'olá', 'ola', 'bom dia', 'boa tarde', 'boa noite'].includes(lower)) {
      st.state = 'MENU'; st.data = {}; st.handoff = false;
    } else if (lower === 'cancelar') {
      st.state = 'MENU'; st.data = {};
    ...
    if (reply === null && st.handoff) {
      ...
      await req('POST', '/rest/v1/conversation_state', { phone, unit_id: unit.id, state: st.state, data: st.data, handoff: st.handoff, updated_at: new Date().toISOString() }, { Prefer: 'resolution=merge-duplicates' });
      continue;
    }
    ...
    await req('POST', '/rest/v1/conversation_state',
      { phone, unit_id: unit.id, state: st.state, data: st.data, handoff: st.handoff, updated_at: new Date().toISOString() },
    ...
    if (mode === 'fallback') {
      results.push({ json: { phone, reply: null, mode, userText: text, systemPrompt } });
    } else {
      results.push({ json: { phone, reply, mode, userText: text, systemPrompt } });
    }
  } catch (e) {
