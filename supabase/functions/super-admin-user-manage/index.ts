...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "reset_password") {
      ...
      return new Response(
        JSON.stringify({ success: true, temp_password: tempPass, message: "Senha redefinida" }),
        { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    if (action === "block") {
      const { error } = await supabase.auth.admin.updateUserById(user_id, { ban_duration: "876600h" });
      if (error) throw error;
      await supabase
        .from("unit_users")
    ...
    if (action === "update_email") {
      ...
      return new Response(
        JSON.stringify({ success: true, message: "Email atualizado" }),
        { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
