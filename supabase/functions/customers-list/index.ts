// Edge Function: customers-list
// GET    -> clientes da unidade com pets aninhados
// POST   -> cria cliente (colunas desconhecidas são ignoradas automaticamente)
// PATCH  -> edita cliente (?id=UUID)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const payload: any = { unit_id: unit.id, is_deleted: false };
      for (const f of ['name', 'phone', 'email', 'cpf', 'address', 'notes', 'birth_date', 'instagram']) {
        if (body[f] !== undefined) payload[f] = body[f] === '' ? null : body[f];
      }
      const { data, error } = await writeSafe(supabase, 'customers', payload, false);
    ...
    if (req.method === 'PATCH') {
      ...
      const { error } = await writeSafe(supabase, 'customers', payload, true, id, unit.id);
      if (error) return json({ success: false, error: error.message }, 500);
